...
public class BigQuerySourceConfig implements Serializable {
    ...
    @FieldDoc(required = false, defaultValue = "", help =
            "The SQL query on BigQuery and save the computed result in a temporary table."
                    + "The temporary table has a configurable expiration time,and the source-connector automatically "
                    + "deletes the temporary table when the data transfer is completely."
                    + "The projectId and datasetName use values in config,and the tableName will be generated by UUID.")
    private String sql;

    @FieldDoc(required = false, defaultValue = "1440", help = "The expiration time in minutes until the table is "
            + "expired and auto-deleted.")
